class Chef::Node
    ...
    def ensure_iptables_will_run_after current_cookbook
        Chef::Application.fatal!("The iptables resources declared in #{current_cookbook} can't be enforced since iptables doesn't run afterwards!") unless recipes.slice(recipes.index(current_cookbook), recipes.length).include?('wk-iptables')
    end
end
